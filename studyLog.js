 ...
 *
 *  この中身は 5つのアプリ（Qalc／漢字タウン／けいさんカード／
 *  さんすうブロック／100マス計算）で **同一** にします。
 *  直すときは 1本だけ直すのではなく、5本すべてに 同じ内容を 配ってください。
 *
 *  【このアプリでの ただ1つの ちがい】
 *    仕様書は `export function saveStudyRecord` ですが、けいさんカードは
 *    ビルドをせず <script> で読みこむ構成のため、ES モジュールの export が
 *    つかえません。中身は変えずに、window（globalThis）に
 *    `StudyLog.saveStudyRecord` として ぶら下げています。
 *
 *  【保存先】
...
(function (global) {
  ...
  function saveStudyRecord(rec) {
    try {
      ...
      //   新しい配列から やりなおします。ここで あきらめてしまうと、
      //   一度こわれた端末は それ以降 ずっと 1件も 保存できなくなります。
